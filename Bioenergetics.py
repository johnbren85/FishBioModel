...
class Batch:
        def __init__(self, Site, Month, Year, k, DaphSize, TotalDaphnia, StartingMass, Dmax, Dmin):
                self.Site = Site
                self.Month = Month
                self.Year = Year
                self.k = k
...
#July 2013 and July 2014 is average of June and August from same year
                ...
                self.DaphWeightdry = (exp(1.468 + 2.83 * log(DaphSize))) / 1000000  # Based of Cornell equation (g) #WetDaphWeight <- DaphWeight*(8.7/0.322) #From Ghazy, others use ~10%
...
# Foraging from Beauchamps paper, prey per hour
        def compute_foragingbydepth(self, StartingLength, StartingMass, surface_light, daphline, daph_auc, depth):
                light = surface_light * exp((-self.k) * depth)
                depth = depth
...
# Units are g/g/d
        ...
        def Run_Batch(self):
                ...
                #Checked fish lengths against this and by end of summer fish weigh much less than they 'should' based on their length
                        ...
                        #self.out['dailyconsume'].append(dailyconsume)
