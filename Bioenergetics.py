...
class Batch:
	def __init__(self, Site, Month, Year, k, DaphSize, TotalDaphnia, StartingMass):
		self.Site = Site
		self.Month = Month
		self.Year = Year
		self.k = k
		...
		if self.k == None and self.Site =='Fall Creek' and self.Year == '2015':
			self.k = FCk[Month]
		elif self.k == None and self.Site =='Hills Creek' and self.Year == '2015':
			self.k = HCk[Month]
		elif self.k == None and self.Site  == 'Lookout Point' and self.Year == '2015':
			 self.k = LPk[Month]
		elif self.k == None and self.Site  =='Fall Creek' and self.Year == '2014':
			 self.k = FCk14[Month]
		elif self.k == None and self.Site =='Hills Creek' and self.Year == '2014':
			 self.k = HCk14[Month]
		elif self.k == None and self.Site  == 'Lookout Point' and self.Year == '2014':
			 self.k = LPk14[Month]
	...
	def Sensitivity_Expand(S_param, Sparam_Range, Sparam_Exp):
		step_size = Sparam_Range/10
	...
	def compute_daphniabydepth(zooplankton_data):
...
# Foraging from Beauchamps paper, prey per hour
	def compute_foragingbydepth(StartingLength, StartingMass, surface_light, daphline, daph_auc, depth):
		light = surface_light * exp((-k) * depth)
		depth = depth
		daphnia = daphline(depth) / 10000
	 	reactiondistance = 3.787 * (light ** 0.4747) * ((self.DaphSize / 10) ** 0.9463)
	 	swim_speed = SwimSpeed * StartingLength
	 	searchvolume = pi * (reactiondistance ** 2) * swim_speed
	 	EncounterRate = searchvolume * daphnia
	 	gramsER = EncounterRate * DaphWeight
	 	return gramsER / StartingMass


	def compute_ft(params, temperature):
		CQ = params['CQ']
		CTL = params['CTL']
		CTM = params['CTM']
		CTO = params['CTO']
	...
	def compute_cmax(params, W):
		CA = params['CA']
		CB = params['CB']
		return CA * (W ** CB)


	def compute_consumption(cmax, P, ft):
		return cmax * P * ft

	def compute_waste(params, consumption, P, temperature, prey, digestibility):
# Units are g/g/d
		FA = params['FA']
		FB = params['FB']
		FG = params['FG']
	...
	def compute_respiration(params, W0, temperature, egestion, consumption):
		RA = params['RA']
		RB = params['RB']
		RQ = params['RQ']
		RTO = params['RTO']
	...
	def predatorenergy(params, W0):
		AlphaI = params['AlphaI']
		AlphaII = params['AlphaII']
		BetaI = params['BetaI']
		BetaII = params['BetaII']
	...
	def compute_bioenergetics(W, temp, P, prey, digestibility, params):
		cmax = compute_cmax(params, W)
		ft = compute_ft(params, temp)
		consumption = compute_consumption(cmax, P, ft)
		(egestion, excretion) = compute_waste(params, consumption, P, temp, prey, digestibility)
		(respiration, SDAction) = compute_respiration(params, W, temp, egestion, consumption)
		return (consumption, egestion, excretion, respiration, SDAction)


	def compute_growth(consumption, prey, preyenergy, egestion, excretion, SDAction, respiration, predatorenergy, W):
		consumptionjoules = consumption * inner(prey, preyenergy)
		return (consumptionjoules - ((egestion + excretion + SDAction) * inner(prey, preyenergy) + respiration * O2Conv)) / predatorenergy * W

	def best_depth(StartingLength,StartingMass,hours,light):
		depth_arr = arange(min(depths),max(depths),0.1)
		growths = [growth_fn(d,StartingLength,StartingMass,hours,light)[0] for d in depth_arr]
		idx = argmax(growths)
		d = depth_arr[idx]
	 	best_growth,best_consumption = growth_fn(d,StartingLength,StartingMass,hours,light)
		return depth_arr[idx], best_growth, best_consumption

	def plot_growth():
		depth_arr = arange(min(depths),max(depths),0.1)
		gs_d = [growth_fn(d,out['StartingLength'][0],out['StartingMass'][0],day_hours,DayLight) for d in depth_arr]
		gs_n = [growth_fn(d, out['StartingLength'][0], out['StartingMass'][0], night_hours, NightLight) for d in depth_arr]
		pylab.plot(depth_arr,gs_d)
		pylab.plot(depth_arr,gs_n)

	def growth_fn(depth, StartingLength, StartingMass, hours, light):
		temp = temp_from_depth(depth)
		foraging = compute_foragingbydepth(StartingLength, StartingMass, light, daphline, daph_auc, depth) * hours
		ft = compute_ft(params, temp)
		cmax = compute_cmax(params, StartingMass)
	 	P = min(foraging / cmax, 1)

	 	(consumption, egestion, excretion, respiration, SDAction) = compute_bioenergetics(StartingMass, temp, P, prey, digestibility, params)
		day_proportion = hours / 24.0
		consumption *= day_proportion
		respiration *= day_proportion
		growth = compute_growth(consumption, prey, preyenergy, egestion, excretion, SDAction,
		respiration, predatorenergy, StartingMass)
